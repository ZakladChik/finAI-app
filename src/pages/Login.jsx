      ...
      <div className="bg-white rounded-3xl shadow-2xl p-10 w-full max-w-md transform transition-all duration-300 hover:shadow-3xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#1e3a5f] text-white py-3 rounded-xl hover:bg-[#2a4a6d] transition-all transform hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed font-semibold"
          >
        ...
        <p className="text-center text-sm text-gray-500 mt-6">
          {t('auth.no_account')}{' '}
          <button onClick={() => onNavigate('register')} className="text-[#d4af37] font-semibold hover:underline transition-all">
            {t('auth.go_register')}
          </button>
        </p>
      </div>
